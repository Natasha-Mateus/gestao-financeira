...
export function renderCartaoVale(container) {
  ...
  async function render() {
    container.innerHTML = `
      ...
      <div class="card" style="margin-bottom:24px">
        <h3>Valores recebidos no mês</h3>
        <div class="grid grid-2" style="margin-top:12px; max-width:500px">
          <div><label>Saldo Livre</label><input type="number" step="0.01" id="vale-valor-livre"></div>
          <div><label>Saldo Voucher</label><input type="number" step="0.01" id="vale-valor-voucher"></div>
        </div>
    ...
    if (configExistente) {
      document.getElementById('vale-valor-livre').value = configExistente.data().valorMensalLivre || '';
      document.getElementById('vale-valor-voucher').value = configExistente.data().valorMensalVoucher || '';
    }
    ...
    document.getElementById('btn-salvar-vale').addEventListener('click', async () => {
      ...
      if (configExistente) {
        await updateDoc(doc(db, 'valeAlimentacaoConfig', configExistente.id), { valorMensalLivre, valorMensalVoucher });
      } else {
    ...
    function renderTipo(elId, tipo, valorMensal, compras) {
      const usadas = compras.filter(c => c.formaPagamento === 'vale' && c.valeTitular === 'Natasha' && (c.valeTipo || 'livre') === tipo && monthRefFromDate(c.data) === mes);
      ...
      document.getElementById(elId).innerHTML = `
        ...
        <div class="ledger-figure" style="margin-top:8px"><div class="value" style="color:${restante >= 0 ? 'var(--olive)' : 'var(--terracota)'}">${formatBRL(restante)}</div><div class="label">Saldo restante</div></div>
        <div style="margin-top:12px; background:var(--surface-2); border-radius:6px; height:8px; overflow:hidden">
          <div style="width:${percentual}%; height:100%; background:${percentual >= 90 ? 'var(--terracota)' : 'var(--gold)'}"></div>
        </div>
        <p style="color:var(--text-dim); font-size:12px; margin-top:10px">${usadas.length} compra(s) nesse mês</p>
      `;
    }

    const unsub = onSnapshot(query(mercadoComprasCol), (snap) => {
      const compras = snap.docs.map(d => d.data());
      const valorLivre = configExistente ? (configExistente.data().valorMensalLivre || 0) : 0;
      const valorVoucher = configExistente ? (configExistente.data().valorMensalVoucher || 0) : 0;
      renderTipo('vale-resumo-livre', 'livre', valorLivre, compras);
      renderTipo('vale-resumo-voucher', 'voucher', valorVoucher, compras);
    });
    unsubs.push(unsub);
  }
