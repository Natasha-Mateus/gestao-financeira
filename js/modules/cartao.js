...
import {
  ...
  addMonths, calcularMesFatura, monthPickerHTML, wireMonthPicker, formatCartao, formatConta, CATEGORIAS_GASTO,
  collapsibleHeaderHTML, wireCollapsible, valorValeDaCompra
} from '../helpers.js';
...
const faturaStatusCol = collection(db, 'faturaStatus');
...
export function renderCartaoCartoes(container) {
  ...
  function renderLista() {
    ...
    el.innerHTML = cartoesAtuais.map(c => {
      ...
      const statusFatura = faturaStatusAtuais.find(f => f.cartaoId === c.id && f.mes === mes);
      const faturaPaga = !!statusFatura;
        ...
        <div class="card" style="margin-bottom:16px">
          <div style="display:flex; justify-content:space-between; align-items:center; cursor:pointer" data-toggle-cartao="${c.id}">
            <div>
              ...
              <span class="tag" style="color:${faturaPaga ? 'var(--olive)' : 'var(--terracota)'}">${faturaPaga ? 'fatura paga' : 'fatura em aberto'}</span>
            </div>
          ...
          ${aberto ? `
            <div style="margin-top:16px; border-top:1px solid var(--border); padding-top:16px">
              ...
              <div style="margin-top:12px; display:flex; justify-content:space-between; align-items:center">
                <div>
                  ${faturaPaga
                    ? `<span style="color:var(--text-dim); font-size:13px">Paga com ${statusFatura.contaNome || '-'} — ${formatBRL(statusFatura.valorPago)}</span> <button class="btn-ghost" data-reabrir-fatura="${c.id}">Reabrir</button>`
                    : `<button class="btn" data-pagar-fatura="${c.id}">Marcar fatura como paga</button>`
                  }
                </div>
                <div>
    ...
    el.querySelectorAll('[data-pagar-fatura]').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const c = cartoesAtuais.find(x => x.id === btn.dataset.pagarFatura);
        const lancsDoCartao = lancamentosAtuais.filter(l => l.cartaoId === c.id);
        let totalFatura = 0;
        lancsDoCartao.forEach(l => {
          parcelasPorCompetencia(l, c).forEach(p => { if (p.mes === mes) totalFatura += p.valor; });
        });
        recorrentesAtuais.filter(r => r.cartaoId === c.id && r.mesInicio && r.mesInicio <= mes).forEach(r => { totalFatura += r.valor; });
        const contas = await getContas();
        abrirModalPagarFatura(c, totalFatura, contas);
      });
    });
    el.querySelectorAll('[data-reabrir-fatura]').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation();
        const statusFatura = faturaStatusAtuais.find(f => f.cartaoId === btn.dataset.reabrirFatura && f.mes === mes);
        if (statusFatura) {
          await ajustarSaldoConta(statusFatura.contaPagamentoId, statusFatura.valorPago);
          await deleteDoc(doc(db, 'faturaStatus', statusFatura.id));
        }
      });
    });
  ...
  const unsub4 = onSnapshot(query(faturaStatusCol), (snap) => {
    faturaStatusAtuais = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    renderLista();
  });
  unsubs.push(unsub4);

  function abrirModalPagarFatura(cartao, totalFatura, contas) {
    const overlay = document.createElement('div');
    overlay.className = 'modal-overlay';
    overlay.innerHTML = `
      <div class="modal-box">
        <h3>Confirmar pagamento da fatura</h3>
        <p style="color:var(--text-dim); font-size:13px; margin-top:-8px; margin-bottom:16px">${formatCartao(cartao)} — fatura de ${mes}</p>
        <label>Conta de saída</label>
        <select id="modal-conta-fatura">
          <option value="">Selecione a conta</option>
          ${contas.map(cc => `<option value="${cc.id}">${formatConta(cc)}</option>`).join('')}
        </select>
        <label>Valor pago</label>
        <input type="number" step="0.01" id="modal-valor-fatura" value="${totalFatura}">
        <div style="display:flex; gap:8px; justify-content:flex-end; margin-top:16px">
          <button class="btn-ghost" id="modal-cancelar-fatura">Cancelar</button>
          <button class="btn" id="modal-confirmar-fatura">Confirmar pagamento</button>
        </div>
      </div>
    `;
    ...
    overlay.querySelector('#modal-cancelar-fatura').addEventListener('click', fechar);

    overlay.querySelector('#modal-confirmar-fatura').addEventListener('click', async () => {
      const contaId = overlay.querySelector('#modal-conta-fatura').value;
      const valorPago = parseFloat(overlay.querySelector('#modal-valor-fatura').value) || 0;
      if (!contaId) { alert('Selecione de qual conta saiu o pagamento.'); return; }
      const conta = contas.find(cc => cc.id === contaId);
      await addDoc(faturaStatusCol, {
        cartaoId: cartao.id, mes, status: 'pago',
        contaPagamentoId: contaId, contaNome: conta ? formatConta(conta) : '', valorPago
      });
      const novoSaldo = await ajustarSaldoConta(contaId, -valorPago);
      if (novoSaldo !== null && novoSaldo < 0) {
        alert('Fatura paga. Atenção: o saldo dessa conta ficou negativo.');
      }
      fechar();
    });
  }
