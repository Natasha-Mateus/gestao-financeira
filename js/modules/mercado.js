...
export async function renderMercadoCompra(container) {
  ...
  container.innerHTML = `
    ...
    <div class="card" style="margin-bottom:24px">
      ...
      <div id="form-compra-body" class="collapsible-body">
  ...
  async function salvarCompra() {
    ...
    document.querySelectorAll('[data-item-id]').forEach(row => {
      ...
      const preco = parseFloat(row.querySelector('.i-preco').value) || 0;
      itens.push({
        nome, categoria: row.querySelector('.i-categoria').value,
      ...
      for (const item of itens) {
        if (item.recorrente) {
          await addDoc(itensEmUsoCol, {
            itemNome: item.nome, categoria: item.categoria, compraId,
            dataCompra: data, status: 'em_uso', dataAcabou: null
          });
        }
      }
    }
...
export function renderMercadoEmUso(container) {
  ...
  function render() {
    container.innerHTML = `
      ...
      ${monthPickerHTML(mes)}
      <p style="color:var(--text-dim); font-size:13px; margin-bottom:16px">Mostrando itens que entraram em uso em ${mes}. Itens ainda em uso de meses anteriores continuam ativos, mas aparecem no mês em que foram comprados.</p>
    ...
    const unsub = onSnapshot(query(itensEmUsoCol), (snap) => {
      ...
      const emUso = itens.filter(i => i.status === 'em_uso' && monthRefFromDate(i.dataCompra) === mes)
        .sort((a, b) => a.dataCompra.localeCompare(b.dataCompra));
      const el = document.getElementById('lista-em-uso');
      if (!emUso.length) {
        el.innerHTML = '<div class="empty-state">Nenhum item em uso registrado nesse mês.</div>';
        return;
      }
      el.innerHTML = `
        <table>
...
export function renderMercadoLista(container) {
  ...
  container.innerHTML = `
    ...
    <div class="card" style="margin-bottom:24px">
      ...
      <div id="form-livre-body" class="collapsible-body">
