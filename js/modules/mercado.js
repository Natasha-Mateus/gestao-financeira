...
export function renderMercadoDespensa(container) {
  ...
  container.innerHTML = `
    ...
    <button class="btn" id="btn-abrir-form-despensa" style="margin-bottom:20px">+ Adicionar item</button>
    <div id="lista-despensa"></div>
  `;
  wireCollapsible(container);

  document.getElementById('btn-abrir-form-despensa').addEventListener('click', () => {
  ...
  const unsub = onSnapshot(query(despensaCol), (snap) => {
    ...
    el.innerHTML = `
      <table>
        <thead><tr><th>Item</th><th>Início</th><th>Fim</th><th>Duração</th><th>Frequência de compra (mês)</th><th>Status</th><th>Ações</th></tr></thead>
        <tbody>
          ${itens.map(i => {
            ...
            <tr>
              <td>
                <div>${i.nome}${i.marca ? ' - ' + i.marca : ''}</div>
                <div style="color:var(--text-dim); font-size:12px">${i.quantidadeAtual ?? '-'} ${i.unidade || ''}</div>
              </td>
