...
export function renderRenda(container) {
  ...
  container.innerHTML = `
    <h2 class="module-title">Receitas</h2>
  ...
  document.getElementById('btn-add-conta').addEventListener('click', async () => {
    ...
    if (editandoContaId) {
      ...
      await registrarHistorico({ modulo: 'Receitas', entidade: 'Conta', acao: 'editar', descricao: `Conta editada: ${nome} (${pessoa} - ${tipo})` });
    } else {
      ...
      await registrarHistorico({ modulo: 'Receitas', entidade: 'Conta', acao: 'criar', descricao: `Conta criada: ${nome} (${pessoa} - ${tipo}), saldo inicial ${formatBRL(saldoInicial)}` });
    }
    resetFormConta();
  });
  ...
  document.getElementById('btn-add-reserva').addEventListener('click', async () => {
    ...
    if (editandoReservaId) {
      ...
      await registrarHistorico({ modulo: 'Receitas', entidade: 'Reserva', acao: 'editar', descricao: `Reserva editada: ${payload.nome} (${payload.pessoa}), valor ${formatBRL(payload.valorAtual)}` });
    } else {
      await addDoc(reservasCol, payload);
      await registrarHistorico({ modulo: 'Receitas', entidade: 'Reserva', acao: 'criar', descricao: `Reserva criada: ${payload.nome} (${payload.pessoa}), valor ${formatBRL(payload.valorAtual)}` });
    }
    resetFormReserva();
  });
  ...
  document.getElementById('btn-add-entrada').addEventListener('click', async () => {
    ...
    if (editandoEntradaId) {
      ...
      await registrarHistorico({ modulo: 'Receitas', entidade: 'Entrada', acao: 'editar', descricao: `Entrada editada: ${formatBRL(valor)}${payload.descricao ? ' - ' + payload.descricao : ''} (${data})` });
    } else {
      await addDoc(entradasCol, payload);
      await registrarHistorico({ modulo: 'Receitas', entidade: 'Entrada', acao: 'criar', descricao: `Entrada criada: ${formatBRL(valor)}${payload.descricao ? ' - ' + payload.descricao : ''} (${data})${previsto ? ' [prevista]' : ''}` });
    }
    if (efetivada) {
  ...
  document.getElementById('btn-add-receita-rec').addEventListener('click', async () => {
    ...
    if (editandoReceitaRecId) {
      ...
      await registrarHistorico({ modulo: 'Receitas', entidade: 'Receita recorrente', acao: 'editar', descricao: `Receita recorrente editada: ${nome}, ${formatBRL(valor)}/mês` });
    } else {
      await addDoc(receitasRecorrentesCol, payload);
      await registrarHistorico({ modulo: 'Receitas', entidade: 'Receita recorrente', acao: 'criar', descricao: `Receita recorrente criada: ${nome}, ${formatBRL(valor)}/mês` });
    }
    resetFormReceitaRec();
  });
  ...
  function renderListaReceitasRec(receitas) {
    ...
    el.querySelectorAll('[data-marcar-receita-rec]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        await registrarHistorico({ modulo: 'Receitas', entidade: 'Receita recorrente', acao: 'editar', descricao: `"${r ? r.nome : ''}" marcada como recebida em ${mesRec}` });
      });
    });
    el.querySelectorAll('[data-desmarcar-receita-rec]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (statusDoc) {
          ...
          await registrarHistorico({ modulo: 'Receitas', entidade: 'Receita recorrente', acao: 'editar', descricao: `"${r ? r.nome : ''}" desmarcada como recebida em ${mesRec}` });
        }
      });
    });
    ...
    el.querySelectorAll('[data-del-receita-rec]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (confirm('Remover esta receita recorrente?')) {
          ...
          await registrarHistorico({ modulo: 'Receitas', entidade: 'Receita recorrente', acao: 'remover', descricao: `Receita recorrente removida: ${r ? r.nome : btn.dataset.delReceitaRec}` });
        }
      });
    });
  }
  ...
  function renderListaContas(contas) {
    ...
    el.querySelectorAll('.input-saldo').forEach(inp => {
      inp.addEventListener('change', async () => {
        ...
        await registrarHistorico({ modulo: 'Receitas', entidade: 'Conta', acao: 'editar', descricao: `Saldo ajustado direto: ${c ? formatConta(c) : ''} → ${formatBRL(valorNovo)}` });
      });
    });
    ...
    el.querySelectorAll('[data-del-conta]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (confirm('Remover esta conta?')) {
          ...
          await registrarHistorico({ modulo: 'Receitas', entidade: 'Conta', acao: 'remover', descricao: `Conta removida: ${c ? formatConta(c) : btn.dataset.delConta}` });
        }
      });
    });
  }
  ...
  function renderListaReservas(reservas) {
    ...
    el.querySelectorAll('.input-reserva-valor').forEach(inp => {
      inp.addEventListener('change', async () => {
        ...
        await registrarHistorico({ modulo: 'Receitas', entidade: 'Reserva', acao: 'editar', descricao: `Valor ajustado direto: ${r ? r.nome : ''} → ${formatBRL(valorNovo)}` });
      });
    });
    ...
    el.querySelectorAll('[data-del-reserva]').forEach(btn => {
      btn.addEventListener('click', async () => {
        ...
        if (confirm('Remover esta caixa de reserva?')) {
          ...
          await registrarHistorico({ modulo: 'Receitas', entidade: 'Reserva', acao: 'remover', descricao: `Reserva removida: ${r ? r.nome + ' (' + r.pessoa + ')' : btn.dataset.delReserva}` });
        }
      });
    });
  }
    ...
    unsubEntradas = onSnapshot(query(entradasCol), (snap) => {
      ...
      el.querySelectorAll('[data-efetivar-entrada]').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          await registrarHistorico({ modulo: 'Receitas', entidade: 'Entrada', acao: 'editar', descricao: `Entrada efetivada manualmente: ${formatBRL(e.valor)}${e.descricao ? ' - ' + e.descricao : ''}` });
        });
      });
      ...
      el.querySelectorAll('[data-del-entrada]').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          await registrarHistorico({ modulo: 'Receitas', entidade: 'Entrada', acao: 'remover', descricao: `Entrada removida: ${formatBRL(e.valor)}${e.descricao ? ' - ' + e.descricao : ''} (${e.data})` });
        });
      });
    });
  ...
  async function efetivarEntradasVencidas() {
    ...
    for (const e of vencidas) {
      await ajustarSaldoConta(e.contaId, e.valor);
      await updateDoc(doc(db, 'entradas', e.id), { efetivada: true });
      await registrarHistorico({ modulo: 'Receitas', entidade: 'Entrada', acao: 'editar', descricao: `Entrada prevista efetivada automaticamente (data chegou): ${formatBRL(e.valor)}${e.descricao ? ' - ' + e.descricao : ''}` });
    }
  }
