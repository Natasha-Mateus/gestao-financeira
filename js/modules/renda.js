...
export function renderRenda(container) {
  ...
  container.innerHTML = `
    ...
    <div class="card" style="margin-bottom:24px">
      ...
      <div id="form-receita-rec-body" class="collapsible-body collapsed">
        <p style="color:var(--text-dim); font-size:13px; margin-top:0">Usadas para calcular a Projeção Financeira do Dashboard em meses futuros (ex: salários). Não geram lançamento automático em Entradas — são só a base do cálculo de projeção.</p>
        ...
        <div id="lista-receitas-rec" style="margin-top:20px"></div>
      </div>
    </div>
  ...
  function renderListaReceitasRec(receitas) {
    ...
    el.innerHTML = `
      ...
      <table>
        <thead><tr><th>Nome</th><th>Valor mensal</th><th>Conta</th><th></th></tr></thead>
        <tbody>
          ${receitas.map(r => {
