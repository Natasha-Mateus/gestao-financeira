...
import { registrarHistorico } from '../historicoService.js';
...
export function renderRenda(container) {
    ...
    <div class="card" style="margin-bottom:24px">
      ...
      <div id="form-entrada-body" class="collapsible-body collapsed">
        ...
        <div class="grid grid-4" style="margin-top:12px">
          ...
          <div><label>Previsto?</label>
            <select id="entrada-previsto">
              <option value="nao">Não — já recebi, soma no saldo agora</option>
              <option value="sim">Sim — ainda não recebi, só entra quando a data chegar</option>
            </select>
          </div>
        </div>
        <div style="display:flex; gap:8px">
  ...
  document.getElementById('btn-add-conta').addEventListener('click', async () => {
    ...
    if (editandoContaId) {
      ...
      await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Conta', acao: 'editar', descricao: `Conta editada: ${nome} (${pessoa} - ${tipo})` });
    } else {
      ...
      await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Conta', acao: 'criar', descricao: `Conta criada: ${nome} (${pessoa} - ${tipo}), saldo inicial ${formatBRL(saldoInicial)}` });
    }
    resetFormConta();
  });
  ...
  document.getElementById('btn-add-reserva').addEventListener('click', async () => {
    ...
    if (editandoReservaId) {
      ...
      await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Reserva', acao: 'editar', descricao: `Reserva editada: ${payload.nome} (${payload.pessoa}), valor ${formatBRL(payload.valorAtual)}` });
    } else {
      await addDoc(reservasCol, payload);
      await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Reserva', acao: 'criar', descricao: `Reserva criada: ${payload.nome} (${payload.pessoa}), valor ${formatBRL(payload.valorAtual)}` });
    }
    resetFormReserva();
  });
  ...
  function resetFormEntrada() {
    ...
    document.getElementById('entrada-previsto').value = 'nao';
  }
  ...
  document.getElementById('btn-add-entrada').addEventListener('click', async () => {
    ...
    const previsto = document.getElementById('entrada-previsto').value === 'sim';
    // Se marcada como prevista mas a data já chegou (ou é hoje/passado), já efetiva na hora.
    const efetivada = !previsto || data <= todayISO();
    const payload = {
      data, contaId, valor, previsto, efetivada,
    ...
    if (editandoEntradaId) {
      ...
      // Reverte o valor antigo do saldo, só se ele já tinha sido efetivado antes.
      if (edicaoEntradaOriginal && edicaoEntradaOriginal.efetivada) {
        await ajustarSaldoConta(edicaoEntradaOriginal.contaId, -edicaoEntradaOriginal.valor);
      }
      await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Entrada', acao: 'editar', descricao: `Entrada editada: ${formatBRL(valor)}${payload.descricao ? ' - ' + payload.descricao : ''} (${data})` });
    } else {
      await addDoc(entradasCol, payload);
      await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Entrada', acao: 'criar', descricao: `Entrada criada: ${formatBRL(valor)}${payload.descricao ? ' - ' + payload.descricao : ''} (${data})${previsto ? ' [prevista]' : ''}` });
    }
    if (efetivada) {
      await ajustarSaldoConta(contaId, valor);
    }
    resetFormEntrada();
  });
  ...
  document.getElementById('btn-add-receita-rec').addEventListener('click', async () => {
    ...
    if (editandoReceitaRecId) {
      ...
      await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Receita recorrente', acao: 'editar', descricao: `Receita recorrente editada: ${nome}, ${formatBRL(valor)}/mês` });
    } else {
      await addDoc(receitasRecorrentesCol, payload);
      await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Receita recorrente', acao: 'criar', descricao: `Receita recorrente criada: ${nome}, ${formatBRL(valor)}/mês` });
    }
    resetFormReceitaRec();
  });
  ...
  function renderListaReceitasRec(receitas) {
    ...
    el.querySelectorAll('[data-marcar-receita-rec]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const r = receitas.find(x => x.id === btn.dataset.marcarReceitaRec);
        await addDoc(receitasRecorrentesStatusCol, { receitaId: btn.dataset.marcarReceitaRec, mes: mesRec });
        await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Receita recorrente', acao: 'editar', descricao: `"${r ? r.nome : ''}" marcada como recebida em ${mesRec}` });
      });
    });
    el.querySelectorAll('[data-desmarcar-receita-rec]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const r = receitas.find(x => x.id === btn.dataset.desmarcarReceitaRec);
        const statusDoc = statusReceitasRecAtuais.find(s => s.receitaId === btn.dataset.desmarcarReceitaRec && s.mes === mesRec);
        if (statusDoc) {
          await deleteDoc(doc(db, 'receitasRecorrentesStatus', statusDoc.id));
          await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Receita recorrente', acao: 'editar', descricao: `"${r ? r.nome : ''}" desmarcada como recebida em ${mesRec}` });
        }
      });
    });
    ...
    el.querySelectorAll('[data-del-receita-rec]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const r = receitas.find(x => x.id === btn.dataset.delReceitaRec);
        if (confirm('Remover esta receita recorrente?')) {
          await deleteDoc(doc(db, 'receitasRecorrentes', btn.dataset.delReceitaRec));
          await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Receita recorrente', acao: 'remover', descricao: `Receita recorrente removida: ${r ? r.nome : btn.dataset.delReceitaRec}` });
        }
      });
    });
  }
  ...
  function renderListaContas(contas) {
    ...
    el.querySelectorAll('.input-saldo').forEach(inp => {
      inp.addEventListener('change', async () => {
        const c = contas.find(x => x.id === inp.dataset.contaId);
        ...
        await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Conta', acao: 'editar', descricao: `Saldo ajustado direto: ${c ? formatConta(c) : ''} → ${formatBRL(valorNovo)}` });
      });
    });
    ...
    el.querySelectorAll('[data-del-conta]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const c = contas.find(x => x.id === btn.dataset.delConta);
        if (confirm('Remover esta conta?')) {
          await deleteDoc(doc(db, 'contasBancarias', btn.dataset.delConta));
          await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Conta', acao: 'remover', descricao: `Conta removida: ${c ? formatConta(c) : btn.dataset.delConta}` });
        }
      });
    });
  }
  ...
  function renderListaReservas(reservas) {
    ...
    el.querySelectorAll('.input-reserva-valor').forEach(inp => {
      inp.addEventListener('change', async () => {
        const r = reservas.find(x => x.id === inp.dataset.reservaId);
        const valorNovo = parseFloat(inp.value) || 0;
        await updateDoc(doc(db, 'reservas', inp.dataset.reservaId), { valorAtual: valorNovo });
        await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Reserva', acao: 'editar', descricao: `Valor ajustado direto: ${r ? r.nome : ''} → ${formatBRL(valorNovo)}` });
      });
    });
    ...
    el.querySelectorAll('[data-del-reserva]').forEach(btn => {
      btn.addEventListener('click', async () => {
        const r = reservas.find(x => x.id === btn.dataset.delReserva);
        if (confirm('Remover esta caixa de reserva?')) {
          await deleteDoc(doc(db, 'reservas', btn.dataset.delReserva));
          await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Reserva', acao: 'remover', descricao: `Reserva removida: ${r ? r.nome + ' (' + r.pessoa + ')' : btn.dataset.delReserva}` });
        }
      });
    });
  }
  ...
  function carregarEntradas() {
    ...
    unsubEntradas = onSnapshot(query(entradasCol), (snap) => {
      ...
      el.innerHTML = `
        ...
        <table>
          <thead><tr><th>Data</th><th>Conta</th><th>Descrição</th><th>Valor</th><th>Status</th><th></th></tr></thead>
          <tbody>
            ${entradas.map(e => {
              const conta = contasAtuais.find(c => c.id === e.contaId);
              const efetivada = e.efetivada !== false;
              return `
              <tr>
                ...
                <td><span class="tag" style="color:${efetivada ? 'var(--olive)' : 'var(--terracota)'}">${efetivada ? 'no saldo' : 'previsto'}</span></td>
                <td>
                  ${!efetivada ? `<button class="btn-ghost" data-efetivar-entrada="${e.id}">Efetivar agora</button>` : ''}
      ...
      el.querySelectorAll('[data-efetivar-entrada]').forEach(btn => {
        btn.addEventListener('click', async () => {
          const e = entradas.find(x => x.id === btn.dataset.efetivarEntrada);
          await updateDoc(doc(db, 'entradas', e.id), { efetivada: true });
          await ajustarSaldoConta(e.contaId, e.valor);
          await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Entrada', acao: 'editar', descricao: `Entrada efetivada manualmente: ${formatBRL(e.valor)}${e.descricao ? ' - ' + e.descricao : ''}` });
        });
      });
      ...
      el.querySelectorAll('[data-editar-entrada]').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          edicaoEntradaOriginal = { contaId: e.contaId, valor: e.valor, efetivada: e.efetivada !== false };
          ...
          document.getElementById('entrada-previsto').value = e.previsto ? 'sim' : 'nao';
          window.scrollTo({ top: 0, behavior: 'smooth' });
        });
      });
      el.querySelectorAll('[data-del-entrada]').forEach(btn => {
        btn.addEventListener('click', async () => {
          ...
          const efetivada = e.efetivada !== false;
          if (!confirm(efetivada ? 'Excluir esta entrada? O valor será removido do saldo da conta.' : 'Excluir esta entrada prevista? Ela não chegou a entrar no saldo.')) return;
          if (efetivada) {
            await ajustarSaldoConta(e.contaId, -e.valor);
          }
          await deleteDoc(doc(db, 'entradas', e.id));
          await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Entrada', acao: 'remover', descricao: `Entrada removida: ${formatBRL(e.valor)}${e.descricao ? ' - ' + e.descricao : ''} (${e.data})` });
        });
      });
    });
  ...
  async function efetivarEntradasVencidas() {
    const snap = await new Promise(res => { const u = onSnapshot(query(entradasCol), s => { res(s); u(); }); });
    const vencidas = snap.docs
      .map(d => ({ id: d.id, ...d.data() }))
      .filter(e => e.previsto && e.efetivada === false && e.data <= todayISO());
    for (const e of vencidas) {
      await ajustarSaldoConta(e.contaId, e.valor);
      await updateDoc(doc(db, 'entradas', e.id), { efetivada: true });
      await registrarHistorico({ modulo: 'Renda e Reservas', entidade: 'Entrada', acao: 'editar', descricao: `Entrada prevista efetivada automaticamente (data chegou): ${formatBRL(e.valor)}${e.descricao ? ' - ' + e.descricao : ''}` });
    }
  }

  refreshMonthPicker();
  efetivarEntradasVencidas().then(() => carregarEntradas());
