...
import { parcelasPorCompetencia } from './cartao.js';
...
export function renderDashboard(container) {
  ...
  function carregarTudo() {
    ...
    function atualizarSaldoTopo() {
      ...
      if (mesesAFrente > 0) {
        calcularProjecaoDetalhada(saldoAtualReal, mesesAFrente).then(proj => {
          ...
          `;
          listaEl.innerHTML = `
            <p style="color:var(--text-dim); font-size:13px">Saldo atual (${formatBRL(saldoAtualReal)}) + receitas recorrentes − contas fixas − variáveis já registradas com data futura − cartão (parcelas e recorrências já lançadas), somado mês a mês até ${mes}.</p>
            <div class="grid grid-3" style="margin-top:12px">
              ...
              <div class="ledger-figure"><div class="value" style="font-size:16px">${formatBRL(proj.totalVariaveisPrevisto)}</div><div class="label">Variáveis já registradas</div></div>
    ...
    async function calcularProjecaoDetalhada(saldoAtualReal, mesesAFrente) {
      ...
      const [snapVar, snapCartoes, snapLancs, snapRec, snapMercado] = await Promise.all([
        new Promise(res => { const u = onSnapshot(query(collection(db, 'variaveisDespesas')), s => { res(s); u(); }); }),
        ...
        new Promise(res => { const u = onSnapshot(query(collection(db, 'mercadoCompras')), s => { res(s); u(); }); })
      ]);

      const variaveis = snapVar.docs.map(d => d.data());
      ...
      const comprasMercado = snapMercado.docs.map(d => d.data());

      let totalFixasPrevisto = 0, totalVariaveisPrevisto = 0, totalCartaoPrevisto = 0, totalMercadoPrevisto = 0;

      mesesAlvo.forEach(mAlvo => {
        totalFixasPrevisto += totalFixasTemplateSum;

        totalVariaveisPrevisto += variaveis
          .filter(v => monthRefFromDate(v.data) === mAlvo)
          .reduce((s, v) => s + (v.valorTotal || 0), 0);

        totalMercadoPrevisto += comprasMercado
          .filter(c => monthRefFromDate(c.data) === mAlvo && c.formaPagamento !== 'vale')
      ...
      const totalSaidasPrevistas = totalFixasPrevisto + totalVariaveisPrevisto + totalCartaoPrevisto + totalMercadoPrevisto;
      const saldoProjetado = saldoAtualReal + totalEntradasPrevistas - totalSaidasPrevistas;

      return { saldoProjetado, totalEntradasPrevistas, totalSaidasPrevistas, totalFixasPrevisto, totalVariaveisPrevisto, totalCartaoPrevisto: totalCartaoPrevisto + totalMercadoPrevisto };
    }
    ...
    const u1 = onSnapshot(query(collection(db, 'mercadoCompras')), (snap) => {
      ...
      ultimoComprasVale = comprasVale;
      atualizarCardVale();
      atualizarResumo(); atualizarCategorias();
    });
    cachedUnsubs.push(u1);

    let valeValorLivre = 0, valeValorVoucher = 0, ultimoComprasVale = [];
    const u1b = onSnapshot(query(collection(db, 'valeAlimentacaoConfig')), (snap) => {
      ...
      valeValorLivre = configDoMes ? (configDoMes.valorMensalLivre || 0) : 0;
      valeValorVoucher = configDoMes ? (configDoMes.valorMensalVoucher || 0) : 0;
      atualizarCardVale();
    });
    cachedUnsubs.push(u1b);

    function figuraVale(tipo, valorMensal) {
      const usadas = ultimoComprasVale.filter(c => (c.valeTipo || 'livre') === tipo);
      const totalUsado = usadas.reduce((s, c) => s + (c.valorTotal || 0), 0);
      ...
      return `
        <div>
          ...
          <div class="grid grid-2">
            <div class="ledger-figure"><div class="value">${formatBRL(totalUsado)}</div><div class="label">Gasto (${usadas.length})</div></div>
            <div class="ledger-figure"><div class="value" style="color:${restante >= 0 ? 'var(--olive)' : 'var(--terracota)'}">${formatBRL(restante)}</div><div class="label">Restante</div></div>
          </div>
    ...
    function atualizarCardVale() {
      document.getElementById('card-vale').innerHTML = `
        <div class="grid grid-2">
          ${figuraVale('livre', valeValorLivre)}
          ${figuraVale('voucher', valeValorVoucher)}
        </div>
        <p style="color:var(--text-dim); font-size:13px; margin-top:12px">Não entra no "Total gasto no mês" do casal — é um benefício à parte.</p>
      `;
    }
        ...
        unsubCartaoRec = onSnapshot(query(collection(db, 'cartaoRecorrentes')), (snapRec) => {
          ...
          recorrentes.forEach(r => {
            ...
            if (r.mesInicio && r.mesInicio <= mes) {
              totalRecorrentes += r.valor;
              if (categoriaFiltro === 'todas' || categoriaFiltro === r.categoria) {
                categoriaTotais[r.categoria] = (categoriaTotais[r.categoria] || 0) + r.valor;
              }
            }
          });
          totalCartaoManual += totalRecorrentes;

          document.getElementById('card-cartao').innerHTML = `
            <div class="ledger-figure"><div class="value">${formatBRL(totalFaturaMes + totalRecorrentes)}</div><div class="label">Total das faturas este mês</div></div>
          `;
          atualizarResumo(); atualizarCategorias();
        });
      });
