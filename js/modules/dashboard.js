...
export function renderDashboard(container) {
  ...
  function carregarTudo() {
    ...
    let totalFixasPendentes = 0, totalCartaoCompetenciaMes = 0, totalReceitasRecorrentesPendentes = 0;
    let contasAtuais = [];
    let categoriaTotais = {};
    ...
    function atualizarSaldoTopo() {
      ...
      const saldoPrevisto = saldoAtualReal + totalReceitasRecorrentesPendentes - totalFixasPendentes - totalCartaoCompetenciaMes;
      document.getElementById('saldo-resumo-topo').innerHTML = `
      ...
      listaEl.innerHTML = `
        ...
        <p style="color:var(--text-dim); font-size:13px; margin-top:12px">"Saldo previsto" = saldo atual + receitas recorrentes ainda não recebidas − contas fixas pendentes − cartão (parcelas e recorrências do mês). Variáveis/Mercado à vista já registrados não entram de novo aqui — já estão refletidos no saldo atual.</p>
      `;
    }
    ...
    const u2 = onSnapshot(query(collection(db, 'cartoes')), (snapCartoes) => {
      ...
      const u2b = onSnapshot(query(collection(db, 'cartaoLancamentos')), (snap) => {
        ...
        unsubCartaoRec = onSnapshot(query(collection(db, 'cartaoRecorrentes')), (snapRec) => {
          ...
          totalCartaoCompetenciaMes = totalFaturaMes + totalRecorrentes;

          document.getElementById('card-cartao').innerHTML = `
            <div class="ledger-figure"><div class="value">${formatBRL(totalCartaoCompetenciaMes)}</div><div class="label">Total das faturas este mês</div></div>
          `;
          atualizarResumo(); atualizarCategorias();
        });
      });
