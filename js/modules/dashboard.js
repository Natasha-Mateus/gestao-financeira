...
import {
  ...
  monthPickerHTML, wireMonthPicker, CATEGORIAS_GASTO
} from '../helpers.js';
  ...
  function carregarTudo() {
    ...
    function atualizarResumo() {
      ...
      document.getElementById('resumo-topo').innerHTML = `
        <div class="ledger-figure"><div class="value">${formatBRL(totalRenda)}</div><div class="label">Renda do mês</div></div>
        <div class="ledger-figure"><div class="value">${formatBRL(totalGastos)}</div><div class="label">Total gasto no mês</div></div>
        <div class="ledger-figure"><div class="value" style="color:${sobra >= 0 ? 'var(--olive)' : 'var(--terracota)'}">${formatBRL(sobra)}</div><div class="label">Sobra estimada</div></div>
      `;
    }
    ...
    const u1 = onSnapshot(query(collection(db, 'mercadoCompras')), (snap) => {
      ...
      document.getElementById('card-vale').innerHTML = `
        <div class="ledger-figure"><div class="value">${formatBRL(totalVale)}</div><div class="label">${comprasVale.length} compra(s) pagas com o vale este mês</div></div>
        <p style="color:var(--text-dim); font-size:13px; margin-top:10px">Esse valor não entra no "Total gasto no mês" do casal — é um benefício à parte. Veja o saldo detalhado em Cartão › Vale Alimentação.</p>
      `;
    ...
    const u4 = onSnapshot(query(collection(db, 'contasFixas')), (snapT) => {
      ...
      const u4b = onSnapshot(query(collection(db, 'contasFixasStatus')), (snapS) => {
        ...
        if (pessoaFiltro !== 'todos') filtrados = filtrados.filter(t => t.responsavel === pessoaFiltro || t.responsavel === 'Dividido');
    ...
    // Itens em uso há muito tempo (alerta)
    const u5 = onSnapshot(query(collection(db, 'itensEmUso')), (snap) => {
    ...
    const u6 = onSnapshot(query(collection(db, 'contasBancarias')), (snap) => {
      let contas = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    ...
    const u7 = onSnapshot(query(collection(db, 'contasBancarias')), (snapContas) => {
      ...
      const u7b = onSnapshot(query(collection(db, 'rendaMensal')), (snap) => {
        let rendas = snap.docs.map(d => d.data()).filter(r => r.mes === mes);
        if (pessoaFiltro !== 'todos') {
          rendas = rendas.filter(r => {
