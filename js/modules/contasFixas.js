...
export function renderContasFixas(container) {
  ...
  container.innerHTML = `
    ...
    <div class="card" style="margin-bottom:24px">
      <div class="ledger-figure"><div class="value" id="cf-total-mensal">R$ 0,00</div><div class="label">Total mensal de contas fixas</div></div>
    </div>
    <div class="card" style="margin-bottom:24px">
  ...
  async function carregarLista() {
    ...
    function render() {
          ...
          <table>
            ...
            <tbody>
              ${templates.map(t => {
                ...
                const corStatus = status === 'pago' ? 'var(--olive)' : status === 'ignorado' ? 'var(--text-dim)' : 'var(--terracota)';
                return `
                <tr>
                  <td>${t.nome}</td>
                  <td>${formatBRL(t.valor)}</td>
                  <td>dia ${t.diaVencimento || '-'}</td>
                  <td><span class="tag" style="color:${corStatus}">${status === 'ignorado' ? 'removida do mês' : status}</span></td>
                  ...
                  <td>
                    ${status === 'pago'
                      ...
                      : status === 'ignorado'
                        ? `<button class="btn-ghost" data-desfazer-ignorar="${t.id}">Desfazer</button>`
                        : `<button class="btn" data-pagar="${t.id}">Marcar como pago</button>
                           <button class="btn-ghost" data-ignorar="${t.id}" title="Use quando essa conta for paga por outro meio nesse mês, ex: cartão">Remover deste mês</button>`
                    }
      ...
      listaEl.querySelectorAll('[data-desfazer-ignorar]').forEach(btn => {
        btn.addEventListener('click', async () => {
          const statusDoc = statusDocs.find(s => s.templateId === btn.dataset.desfazerIgnorar && s.mes === mes);
          if (statusDoc) await deleteDoc(doc(db, 'contasFixasStatus', statusDoc.id));
        });
      });
    ...
    unsubTemplates = onSnapshot(query(contasFixasCol), (snap) => {
      ...
      const totalMensal = latestTemplates.reduce((s, t) => s + (t.valor || 0), 0);
      document.getElementById('cf-total-mensal').textContent = formatBRL(totalMensal);
      render();
    });
