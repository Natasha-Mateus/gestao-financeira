...
App pessoal de gestão financeira para o casal, com módulos de Dashboard, Receitas, Contas Fixas, Variáveis, Mercado, Cartão de Crédito e Histórico.
...
## Cartão de Crédito: pagar a fatura
Dentro de Cartão › Cartões, ao expandir um cartão aparece "Marcar fatura como paga" (mês selecionado). Abre modal pedindo conta de saída e valor (pré-preenchido, editável) — confirmar desconta o saldo daquela conta e marca a fatura como paga, com opção de "Reabrir" pra desfazer (devolve o saldo). Nova coleção: `faturaStatus` ({ cartaoId, mes, status, contaPagamentoId, contaNome, valorPago }).

A partir dessa mudança, "Saídas do mês" e "Saldo Previsto" no Dashboard passam a considerar o cartão do mesmo jeito que Contas Fixas: só entra em "Saídas" quando a fatura estiver paga; enquanto pendente, entra como "a pagar" dentro do Saldo Previsto. Isso vale por cartão — se você tem 2 cartões e só paga um, o pago sai de "pendente" e o outro continua lá.

## Menu reordenado
Ordem atual: Dashboard, Receitas, Contas Fixas, Variáveis, Mercado, Cartão de Crédito, Histórico. "Renda e Reservas" foi renomeado para **Receitas** (mesmo conteúdo: contas, entradas, reservas e receitas recorrentes).

## Histórico
Novo módulo (menu lateral, abaixo do Cartão de Crédito) que registra toda alteração feita em Contas, Entradas, Reservas e Receitas Recorrentes — criar, editar, remover, marcar/desmarcar recebida, efetivação automática de entrada prevista. Filtros por mês, tipo de registro, ação e busca por texto na descrição. Coleção nova: `historico` ({ modulo, entidade, acao, descricao, data, timestamp }).
