...
- `contasFixas` — modelos de conta fixa (recorrentes ou únicas, sem campo de responsável)
...
- `mercadoCompras`, `despensaItens` (nome + marca + datas de início/fim + em uso), `listaCompras`
- `cartoes`, `cartaoLancamentos` (campo `origem`: manual, mercado ou variaveis), `valeAlimentacaoConfig`
- `contasBancarias`, `entradas` (lançamentos de entrada de dinheiro — data, conta, valor, descrição opcional, soma ao saldo automaticamente), `reservas` (caixas de reserva)

## Despensa (antigo "Itens em uso")
Todo item comprado no Mercado entra automaticamente na Despensa. Para agrupar corretamente (ex: duas marcas de detergente aparecerem como um só "Detergente"), o campo **Nome** do item deve ser sempre o nome genérico do produto, e a **Marca** (opcional) fica como detalhe de cada unidade. O agrupamento é feito por nome exato (sem diferenciar maiúsculas/minúsculas).

A Despensa não adiciona nada à Lista de Compras sozinha — isso só acontece quando alguém clica em "Incluir na lista de compras" manualmente, mesmo que a data fim não esteja preenchida.
