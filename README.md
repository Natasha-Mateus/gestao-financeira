...
- `cartoes` (agora com `limite`), `cartaoLancamentos` (campo `origem`: manual, mercado ou variaveis), `cartaoRecorrentes` (assinaturas/planos — aparecem em toda fatura a partir do mês de início), `valeAlimentacaoConfig`
...
Essa lógica é usada em: Cartão › Lançamentos, Cartão › Faturas futuras, Cartão › visão detalhada por cartão, e no total consolidado do Dashboard.
